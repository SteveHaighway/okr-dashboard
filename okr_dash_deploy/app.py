...
DATA = {
    ...
    "key_results": [
        {"id":"KR-1","objective_id":"OBJ-1","title":"Monthly Revenue Growth ($)","unit":"$","owner":"Liam O'Connor","status":"On Track","target":30000,"current":38000,"delta_to_target_pct":30,"series":[
        ...
        {"id":"KR-2","objective_id":"OBJ-1","title":"New Customer Acquisition (Qty)","unit":"qty","owner":"Fatima Masood","status":"Off Track","target":371,"current":285,"delta_to_target_pct":-20,"series":[
        ...
        {"id":"KR-3","objective_id":"OBJ-1","title":"Upsell Revenue ($)","unit":"$","owner":"Jamal Habib","status":"At Risk","target":9000,"current":7000,"delta_to_target_pct":-20,"series":[
        ...
        {"id":"KR-4","objective_id":"OBJ-1","title":"Customer Retention Rate (%)","unit":"%","owner":"Ariya Patel","status":"Off Track","target":61,"current":56,"delta_to_target_pct":-6,"series":[
        ...
        {"id":"KR-5","objective_id":"OBJ-1","title":"Revenue from New Markets ($)","unit":"$","owner":"Michael Brown","status":"On Track","target":24000,"current":39000,"delta_to_target_pct":70,"series":[
        ...
        {"id":"KR-6","objective_id":"OBJ-1","title":"Subscription Renewal Rate (%)","unit":"%","owner":"Olive Brown","status":"On Track","target":50,"current":56,"delta_to_target_pct":6,"series":[
...
def objective_card(obj, active_id):
    ...
    status_dot = html.Span(style={"display":"inline-block","width":"10px","height":"10px","borderRadius":"50%","backgroundColor":STAT_COLORS.get(obj["status"], STAT_COLORS["Default"]),"marginRight":"6px"})
    return dbc.Card(
        dbc.CardBody([
...
def kr_mini_chart(kr):
    ...
    fig = go.Figure()
    fig.add_bar(x=x, y=y, name="Actual", marker_color=colors)
    ...
    fig.update_layout(
        ...
        height=120,
        autosize=True,
        uirevision="static",         # keep layout stable across re-renders
        transition={'duration': 0}   # disable animations
    )
    # prevent axis changes from triggering relayout
...
def kr_card(kr):
    ...
    return dbc.Card(
        dbc.CardBody([
            ...
            stats,
            dcc.Graph(
                id=f"kr-mini-{kr['id']}",          # stable id stops re-attach loops
                figure=kr_mini_chart(kr),
